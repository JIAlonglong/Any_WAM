...
class FlowMatchScheduler():
    ...
    def apply_shift(self, sigmas: torch.Tensor) -> torch.Tensor:
        """Apply the scheduler's SNR shift to arbitrary sigma values."""
        if self.exponential_shift:
            mu = self.exponential_shift_mu
            return math.exp(mu) / (math.exp(mu) + (1 / sigmas - 1))
        if self.shift == 1.0:
            return sigmas
    ...
    def add_noise(self, original_samples, noise, timestep, t_dim=2):
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.cpu()
        # Flatten timestep for lookup, then reshape back
        orig_shape = timestep.shape
        flat_t = timestep.reshape(-1)  # [B] or [B*T]
        # Find nearest timestep index for each element
        timestep_id = torch.argmin(
            (self.timesteps[:, None] - flat_t[None]).abs(), dim=0
        )  # [flat_size]
        # Build sigma with proper broadcasting shape
        shape = [1] * noise.ndim
        if len(orig_shape) == 2:
            ...
            shape[0] = B
            shape[t_dim] = T
            sigma = self.sigmas[timestep_id].to(original_samples).reshape(B, T)
            sigma = sigma.view(shape)
        else:
            # [B] per-sample timesteps
            shape[t_dim] = orig_shape[0]
