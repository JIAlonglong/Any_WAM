...
validate_port_base() {
    ...
    if (( 10#$port + lane_count - 1 > 65535 )); then
...
if ranges_overlap "$S2_MASTER_PORT_BASE" "$((10#$S2_MASTER_PORT_BASE + S2_LANE_COUNT - 1))" "$S4_MASTER_PORT_BASE" "$((10#$S4_MASTER_PORT_BASE + S4_LANE_COUNT - 1))"; then
    echo "S2 and S4 master port ranges must not overlap" >&2
    exit 2
fi
if ranges_overlap "$S2_WS_PORT_BASE" "$((10#$S2_WS_PORT_BASE + S2_LANE_COUNT - 1))" "$S4_WS_PORT_BASE" "$((10#$S4_WS_PORT_BASE + S4_LANE_COUNT - 1))"; then
