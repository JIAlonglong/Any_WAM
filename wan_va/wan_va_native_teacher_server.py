...
class VA_Server:
    ...
    def _infer(self, obs, frame_st_id=0):
        ...
        self.scheduler.set_timesteps(self.native_contract["video_steps"])
        self.action_scheduler.set_timesteps(self.native_contract["action_steps"])
...
def run(args):
    ...
    weight_path = checkpoint_path / "diffusion_pytorch_model.safetensors"
    weight_index_path = (
        checkpoint_path / "diffusion_pytorch_model.safetensors.index.json"
    )
    if (
        checkpoint_path.name != "transformer"
        or not checkpoint_path.is_dir()
        or not (weight_path.is_file() or weight_index_path.is_file())
    ):
        raise FileNotFoundError(
            "--checkpoint-path must be a transformer directory containing "
            "diffusion_pytorch_model.safetensors or "
            "diffusion_pytorch_model.safetensors.index.json: "
            f"{checkpoint_path}"
        )

    config.wan22_pretrained_model_name_or_path = str(checkpoint_path.parent)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the native base model transformer directory.",
    )
    parser.add_argument(
        "--num-steps",
        type=int,
        required=True,
        help="Number of native video denoising steps.",
    )
    parser.add_argument(
        "--action-num-steps",
        type=int,
        required=True,
        help="Number of native action denoising steps.",
    )
    parser.add_argument(
        "--model-name",
        default="teacher_native",
        help="Stable native teacher model identity.",
    )
    parser.add_argument(
        "--latency-jsonl",
        default=None,
        help="Append sampler-only latency records to this JSONL file.",
    )
    parser.add_argument(
        "--save-root",
        required=True,
