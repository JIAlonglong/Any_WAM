...
STUDENT_CKPT="${PROJECT_ROOT}/distillation_flowmap/output_libero/checkpoints/${STEP}/${VARIANT}/transformer"
...
if [ ! -d "$STUDENT_CKPT" ]; then
    ...
    ls "${PROJECT_ROOT}/distillation_flowmap/output_libero/checkpoints/"
    exit 1
fi
...
run_compare() {
    ...
    start_server "$TEACHER_CKPT" "$TEACHER_SAVE" "$NUM_STEPS"
    ...
    start_server "$STUDENT_CKPT" "$STUDENT_SAVE" "$NUM_STEPS"
...
run_success() {
    ...
    start_server "$STUDENT_CKPT" "$ACTION_DIR" "$NUM_STEPS"
