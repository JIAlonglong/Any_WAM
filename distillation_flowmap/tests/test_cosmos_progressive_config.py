...
def test_full_cosmos_anchor_separates_low_noise_student_from_supported_teacher():
    ...
    assert "if mixed_step_selection is not None:" in full_cosmos_block
    assert "teacher_x_r, teacher_v_r, _ = rollout_velocity_field(" in full_cosmos_block
...
def test_progressive_terminal_prior_tolerance_covers_libero_action_roundoff(
    ...
    assert module.cfg.opd_danceopd_terminal_prior_tolerance == pytest.approx(
        2e-6
    )
