...
class FlexAttnFunc(nn.Module):
    ...
    @staticmethod
    @torch.no_grad()
    @torch._dynamo.disable  # init_mask 动态创建 block_mask，不能被 compile
    def init_mask(
        latent_shape,
        action_shape,
        padded_length,
        ...
        """初始化 FlexAttention 的 block mask。
        ...
        action_seq_id = torch.arange(B, device=device)[:, None, None, None].expand(-1, A_F, A_H, A_W).flatten()
        seq_ids = torch.cat([latent_seq_id] * 2 + [action_seq_id] * 2)

        latent_frame_id = torch.arange(L_F, device=device)[None, :, None, None].expand(B, -1, L_H // patch_size[1], L_W // patch_size[2])[None].flatten()
        action_frame_id = torch.arange(A_F, device=device)[None, :, None, None].expand(B, -1, A_H, A_W)[None].flatten()
        ...
        mask_mod = FlexAttnFunc._get_mask_mod(seq_ids.long(), frame_ids.long(), noise_ids.long(), window_size)
        block_mask = FlexAttnFunc.create_block_mask_fn(
                mask_mod, 1, 1, len(seq_ids), len(seq_ids), device=device,
            )
        FlexAttnFunc.attention_mask = block_mask

        text_seq_ids = torch.arange(B, device=device)[:, None].expand(-1, 512).flatten()
        mask_mod_cross = FlexAttnFunc._get_cross_mask_mod(seq_ids.long(), text_seq_ids.long())
        block_mask_cross = FlexAttnFunc.create_block_mask_fn(
                mask_mod_cross, 1, 1, len(seq_ids), len(text_seq_ids), device=device,
            )
        FlexAttnFunc.cross_attention_mask = block_mask_cross

        # --- cache store
        FlexAttnFunc._mask_cache[cache_key] = (block_mask, block_mask_cross)
...
class WanAttention(torch.nn.Module):
    ...
    def restore_cache(self, cache_name, slots):
        cache = self.attn_caches.get(cache_name) if self.attn_caches else None
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward_train(self, input_dict, fdm=False):
        ...
        full_grid_id = full_grid_id.to(hidden_states.device)  # 确保在 GPU 上，避免 torch.compile 警告
