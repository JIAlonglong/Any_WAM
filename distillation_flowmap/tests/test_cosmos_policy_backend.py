...
def test_cosmos_all_cosmos_stage1_resume_env_overrides(monkeypatch):
    ...
    monkeypatch.setenv("RESUME_OPTIMIZER_STATE", "1")
    sys.modules.pop(
        "distillation_flowmap.config_libero_cosmos_policy_stage1_all_cosmos_flowmap", None)
...
def test_cosmos_all_cosmos_stage2_flowmap_config_imports(monkeypatch):
    monkeypatch.setenv("COSMOS_POLICY_PATH", "/tmp/cosmos-policy")
    monkeypatch.setenv("STUDENT_BASE_MODEL_PATH", "/tmp/wanva-base")
    sys.modules.pop(
        "distillation_flowmap.config_libero_cosmos_policy_stage2_all_cosmos_flowmap",
        None,
    )
    ...
    assert cfg.resume_from_path.endswith(
        "output_libero_cosmos_policy_stage1_all_cosmos_flowmap/checkpoints/step_5000"
    )
