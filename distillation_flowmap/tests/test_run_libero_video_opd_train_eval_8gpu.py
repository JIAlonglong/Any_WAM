...
def test_dry_run_serializes_train_and_three_matched_evaluations(tmp_path):
    ...
    assert [line.split()[1] for line in eval_lines] == [
        "teacher",
        "stage1_only",
        "stage2",
    ]
    ...
    assert "EVAL_CONTRACT naive_composition=missing" in result.stdout


def test_dry_run_adds_naive_only_for_explicit_distinct_checkpoint(tmp_path):
    teacher = tmp_path / "teacher"
    stage1 = tmp_path / "stage1"
    naive = tmp_path / "naive"
    for path in (
        teacher / "transformer",
        stage1 / "target_student" / "transformer",
        naive,
    ):
        path.mkdir(parents=True)
    env = os.environ.copy()
    env.update(
        {
            ...
            "OUTPUT_DIR": str(tmp_path / "stage2"),
            "EVAL_OUTPUT_ROOT": str(tmp_path / "eval"),
            "NAIVE_CKPT": str(naive),
        }
    )

    result = subprocess.run(
        ["bash", str(SCRIPT), "--dry-run", "--phase", "eval"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    eval_lines = [
        line for line in result.stdout.splitlines() if line.startswith("EVAL ")
    ]
    assert [line.split()[1] for line in eval_lines] == [
        "teacher",
        "stage1_only",
    ...
    assert f"checkpoint={naive}" in eval_lines[-1]
